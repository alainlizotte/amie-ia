"""Authentification par comptes locaux.

Stockage (dans le data_dir du projet) :
- Comptes : `utilisateurs.json` — map nom → {sel, hash, date_creation}.
  Les mots de passe sont hachés PBKDF2-SHA256 (100k itérations, sel aléatoire).
- Secret de signature : `auth_secret.txt` (généré au premier démarrage,
...
Migration legacy : l'ancien système (`users.json` + SHA-256 sans sel, créé /
vérifié par le premier appel à /api/login) est lu à la connexion : un compte
ancien validé est automatiquement re-haché en PBKDF2 dans le nouveau fichier.
"""
...
def _legacy_users_path(data_dir: str) -> str:
...
# --------------------------------------------------------------------------- #
def _verifier_legacy_sha256(data_dir: str, nom: str, mot_de_passe: str) -> bool:
    """Valide contre l'ancien fichier users.json (première génération)."""
    try:
        with open(_legacy_users_path(data_dir), "r", encoding="utf-8") as f:
            data = json.load(f)
    except (OSError, json.JSONDecodeError):
        return False
    entry = (data.get("users", {}) or {}).get((nom or "").strip().lower())
    if not entry:
        return False
    digest = hashlib.sha256((mot_de_passe or "").encode("utf-8")).hexdigest()
    return hmac.compare_digest(digest, str(entry.get("password_sha256", "")))


def _migrer_utilisateur_legacy(data_dir: str, nom: str, mot_de_passe: str) -> bool:
    """Re-hache un compte legacy validé vers le nouveau format PBKDF2."""
    utilisateurs = _charger_utilisateurs(data_dir)
    cle = (nom or "").strip().lower()
    if any(u.lower() == cle for u in utilisateurs):
        return False  # déjà migré
    sel, h = _hasher_mot_de_passe(mot_de_passe)
    utilisateurs[(nom or "").strip()] = {
        "sel": sel,
        "hash": h,
        "date_creation": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "migre_de": "users.json",
...
def verifier_identifiants(data_dir: str, nom: str, mot_de_passe: str) -> bool:
    ...
    # Compte absent du nouveau fichier : tentative de migration legacy.
    if _verifier_legacy_sha256(data_dir, nom, mot_de_passe):
        _migrer_utilisateur_legacy(data_dir, (nom or "").strip(), mot_de_passe)
        return True
    return False
