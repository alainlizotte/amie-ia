// Page de connexion — onglets Connexion / Inscription : comptes locaux +
// token Bearer mémorisé. Avertissement 18+ : case de déclaration obligatoire
// avant connexion (mémorisée localement pour les visites suivantes).

import { useState } from "react";
import { Navigate, useNavigate } from "react-router-dom";
import { apiConnexion, apiInscription, getToken } from "../api/rest";
...
export function LoginPage() {
  ...
  const setToken = useAmie((s) => s.setToken);
  const navigate = useNavigate();

  // Déjà connecté → liste des rencontres directement.
  if (getToken()) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!majeur) return;
    setErreur(null);
    if (!nom.trim() || mdp.length < 4) {
      setErreur("Saisissez un nom (3-24 caractères) et un mot de passe (4 caractères min).");
      return;
    }
    ...
    try {
      const reponse =
        mode === "connexion"
          ? await apiConnexion(nom.trim(), mdp)
          : await apiInscription(nom.trim(), mdp);
      setToken(reponse.token);
      setUser(reponse.utilisateur);
  ...
  return (
    <div className="flex h-full items-center justify-center overflow-y-auto p-6">
      ...
      >
        ...
        <div className="mb-4 flex overflow-hidden rounded-lg border border-rose-900/50 text-sm">
            ...
            <button
              ...
              className={
                ...
                (mode === m
                  ...
                  : "bg-[#1a0b14] text-rose-200/50 hover:text-rose-200")
              }
              onClick={() => {
                setMode(m);
                setErreur("");
        ...
        {mode === "inscription" && (
          <>
            <label className="mb-1 mt-4 block text-sm font-medium text-rose-200/80">
        ...
        <p className="mt-3 text-center text-xs text-rose-200/40">
          {mode === "connexion"
            ? "Pas encore de compte ? Passez par l'onglet Inscription."
            : "Votre compte reste stocké localement sur ce serveur."}
        </p>
      </form>
    </div>
  );
